...
{
  WheelfootController::WheelfootController()
  {
  }

  bool WheelfootController::onInit()
  {
    if (!loadRLCfg())
    {
      RCLCPP_ERROR(rclcpp::get_logger("WheelfootController"), "Error in loadRLCfg");
      return false;
    }
    if (!loadModel())
    {
      RCLCPP_ERROR(rclcpp::get_logger("WheelfootController"), "Error in loadModel");
      return false;
    }
    ...
    cmd_vel_sub_ = cmd_vel_node_->create_subscription<geometry_msgs::msg::Twist>(
        "/cmd_vel", 10, std::bind(&WheelfootController::cmdVelCallback, this, std::placeholders::_1));
  ...
  void WheelfootController::onStart()
  {
    for (size_t i = 0; i < jointNames_.size(); i++)
    {
      defaultJointAngles_[i] = this->getJointStateValue(jointNames_[i], "position");
      RCLCPP_INFO(rclcpp::get_logger("WheelfootController"), "starting hybridJointHandle: %f", defaultJointAngles_[i]);
    }
  ...
  void WheelfootController::onUpdate()
  {
    switch (mode_)
    {
    case Mode::STAND:
  ...
  void WheelfootController::onStop()
  {
  }
  ...
  void WheelfootController::handleWalkMode()
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("WheelfootController"), "Error robotCfg_.controlCfg.decimation");
      return;
    }
  ...
  bool WheelfootController::loadModel()
  {
    ...
    // Policy session
    RCLCPP_INFO(rclcpp::get_logger("WheelfootController"), "Loading policy from: %s", policyModelPath.c_str());
    ...
    for (size_t i = 0; i < policySessionPtr_->GetInputCount(); i++)
    {
      ...
      RCLCPP_INFO(rclcpp::get_logger("WheelfootController"), "GetInputName: %s", policySessionPtr_->GetInputName(i, allocator));
    ...
    for (size_t i = 0; i < policySessionPtr_->GetOutputCount(); i++)
    {
      ...
      RCLCPP_INFO(rclcpp::get_logger("WheelfootController"), "GetOutputName: %s", policySessionPtr_->GetOutputName(i, allocator));
  ...
  bool WheelfootController::loadRLCfg()
  {
    ...
    try
    {
      ...
      if (error != 0)
      {
        RCLCPP_ERROR(rclcpp::get_logger("WheelfootController"), "Some parameters could not be retrieved. Number of errors: %d", error);
      }
      else
      {
    ...
    {
      // Error handling.
      RCLCPP_ERROR(rclcpp::get_logger("WheelfootController"), "Error in the ControllerCfg: %s", e.what());
      return false;
    }
  ...
  void WheelfootController::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    commands_(0) = (msg->linear.x > 4.0 ? 4.0 : (msg->linear.x < -4.0 ? -4.0 : msg->linear.x));
...
PLUGINLIB_EXPORT_CLASS(robot_controllers::WheelfootController, controller_interface::ControllerInterface)
