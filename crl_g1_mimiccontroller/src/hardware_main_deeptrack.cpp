...
#include <crl_g1_mimiccontroller/CRLG1DeepTrackControllerNode.h>
...
int main(int argc, char** argv) {
    ...
    auto m8 = crl::fsm::make_non_persistent_ps<Machines::ONBOARD, States::DEEPTRACK>(
        [&]() {
            auto deepTrackControllerNode = std::make_shared<crl::g1::mimiccontroller::CRLG1DeepTrackControllerNode<States, Machines, 1>>(model, data, "deeptrack");
            // Ensure FSM client is ready as soon as node is constructed
            try {
                deepTrackControllerNode->initializeFsmClient(t_cols, monitoring);
            } catch (const std::exception& e) {
                RCLCPP_ERROR(rclcpp::get_logger("hardware"), "Failed to initialize DeepTrack FSM client: %s", e.what());
            }
            return deepTrackControllerNode;
        });
