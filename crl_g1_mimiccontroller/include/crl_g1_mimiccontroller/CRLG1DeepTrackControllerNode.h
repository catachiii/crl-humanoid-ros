...
namespace crl::g1::mimiccontroller {
    ...
    /**
     * Specialized controller node for CRLG1DeepTrackController that can trigger
     * FSM transitions when the deep mimic sequence is complete.
     */
    template <typename States, typename Machines, std::size_t N>
    class CRLG1DeepTrackControllerNode final : public CRLG1RLControllerNode<CRLG1DeepTrackController> {
    public:
        using Base = CRLG1RLControllerNode<CRLG1DeepTrackController>;

        CRLG1DeepTrackControllerNode(const std::shared_ptr<crl::humanoid::commons::RobotModel>& model,
        ...
        virtual ~CRLG1DeepTrackControllerNode() = default;
