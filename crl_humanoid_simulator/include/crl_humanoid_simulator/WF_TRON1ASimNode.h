...
namespace crl::humanoid::simulator {
    ...
    class WF_TRON1ASimNode : public crl::humanoid::commons::RobotNode<States, Machines, N> {
        using BaseRobotNode = crl::humanoid::commons::RobotNode<States, Machines, N>;

    public:
    WF_TRON1ASimNode(const std::shared_ptr<crl::humanoid::commons::RobotModel>& model,
            ...
            this->template declare_parameter<std::string>("robot_xml_file", "wf_tron1a_description/xml/scene_crl.xml", simulationParamDesc);
            ...
            elasticBandService_ = this->template create_service<crl_humanoid_msgs::srv::ElasticBand>(
                "elastic_band",
                std::bind(&WF_TRON1ASimNode::elasticBandServiceCallback, this, std::placeholders::_1, std::placeholders::_2)
            );
        ...
        void applyElasticBandForce() {
            ...
            if (baseBodyId < 0) {
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                                    "Could not find base_Link body for elastic band support");
                return;
            }
...
#endif  //CRL_HUMANOID_SIMULATOR_WF_TRON1A_NODE
