...
inline void checkSymmetricLinearSystemResidual(const SparseMatrix &A, const dVector &x, const dVector &b) {
    ...
    if (residual > 1e-8) {
        ...
        print(CRL_LOCO_DATA_FOLDER "/out/A.m", A);
        print(CRL_LOCO_DATA_FOLDER "/out/b.m", b);
        print(CRL_LOCO_DATA_FOLDER "/out/x.m", x);
        print(CRL_LOCO_DATA_FOLDER "/out/residual.m", rhs);
    }
}
